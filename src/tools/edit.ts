	...
	for (let i = 0; i < parts.length; i++) {
		...
		if (part.added || part.removed) {
			...
			if (lastWasChange || nextPartIsChange) {
				...
				if (skipStart > 0) output.push(` ${"".padStart(lineNumWidth, " ")} ...`);
				...
				if (skipEnd > 0) output.push(` ${"".padStart(lineNumWidth, " ")} ...`);
	...
	return {
		...
		execute: async (
			...
			if (!content.includes(oldText)) {
				throw new Error(`Could not find the exact text in ${path}. oldText must match exactly.`);
			}

			const occurrences = content.split(oldText).length - 1;
			if (occurrences > 1) {
				throw new Error(`Found ${occurrences} occurrences of the text in ${path}. Make it unique.`);
			}
